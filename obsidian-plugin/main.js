...
class NullclawView extends obsidian_1.ItemView {
    ...
    async exec(input) {
        ...
        try {
            ...
            if (result.stderr)
                this.println(result.stderr, 'nc-error');
            if (!result.stdout && !result.stderr)
                this.println(`(exit: ${result.exitCode})`, 'nc-info');
        }
        catch (e) {
    ...
    async prefetchAgentResponse(input, args) {
        ...
        if (mIdx >= 0 && mIdx + 1 < args.length)
            message = args.slice(mIdx + 1).join(' ');
        else if (mIdx2 >= 0 && mIdx2 + 1 < args.length)
            message = args.slice(mIdx2 + 1).join(' ');
        if (!message)
            return;
